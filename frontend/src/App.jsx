...
async function startAutonomousMode() {
  ...
  try {
    ...
    if (!response.ok || data?.success === false) {
      throw new Error(data?.message || data?.detail || "Autonomous start failed.");
    }
    ...
    if (data?.wallet_baseline_capture?.success === false) {
      throw new Error(
        data.wallet_baseline_capture?.message ||
        "Agent was not started because the wallet start snapshot could not be captured."
      );
    }
...
async function runAgentCycle() {
  ...
  // IMPORTANT: /autonomous/start captures the immutable live-wallet baseline
  // before the autonomous loop is allowed to execute its first cycle/trade.
  // Never call /agent-cycle first here, otherwise the wallet could change
  // before START VALUE is frozen.
  if (!autonomousMode) {
    await startAutonomousMode();
    return;
  }

  // If the backend agent is already running, just refresh its current state.
...
<div className="panel portfolio-section-panel">
  ...
  <div className="panel portfolio-panel">
    ...
    <div className="metrics autonomous-section">
      ...
      <p>START VALUE........... {walletBaseline ? formatMoney(walletBaseline.total_value_usd) : "NOT CAPTURED"}</p>
      <p>BASELINE DATE......... {walletBaseline ? formatDateTime(walletBaseline.captured_at) : "RUN AGENT TO CAPTURE"}</p>
      ...
      {walletBaseline?.assets?.length > 0 && (
        <div className="baseline-snapshot">
          <p><strong>START WALLET SNAPSHOT</strong></p>
          {walletBaseline.assets.map((asset, index) => (
            <div className="baseline-asset" key={`${asset.symbol}-${index}`}>
              <p>ASSET............... {asset.symbol}</p>
              <p>START AMOUNT........ {asset.balance ?? "N/A"} {asset.symbol}</p>
          ...
          <p>SNAPSHOT LOCK........ IMMUTABLE UNTIL EXPLICIT BACKEND RESET</p>
        </div>
      )}
    </div>
  </div>
...
{agentResult?.risk_control && (
  <div className="metrics strategy-library-box" style={{ marginTop: "24px" }}>
    ...
    <p>BASELINE VALUE...... {walletBaseline ? formatMoney(walletBaseline.total_value_usd) : formatMoney(agentResult.risk_control.baseline_portfolio_value_usd || 0)}</p>
    <p>BASELINE DATE....... {walletBaseline ? formatDateTime(walletBaseline.captured_at) : "N/A"}</p>
        ...
        <div className="panel">
          ...
          <details>
            ...
            <div className="metrics">
              ...
              <p>Starting Value: immutable live-wallet snapshot captured when Run Agent is first started. It does not change on normal stop/start cycles.</p>
