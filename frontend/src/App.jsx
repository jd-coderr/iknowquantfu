...
function App() {
  ...
  function getPortfolioStartValueLabel() {
    if (!walletBaseline || walletBaseline.total_value_usd === null || walletBaseline.total_value_usd === undefined) {
      return "NOT SET — RUN AGENT TO CAPTURE";
    }

    return `${formatMoney(walletBaseline.total_value_usd)} (${formatPortfolioBaselineDate(walletBaseline.captured_at)})`;
  }
...
async function startAutonomousMode() {
  ...
  try {
    ...
    if (!response.ok || data?.success === false || data?.status === "blocked") {
      setAutonomousMode(false);
      setAgentStopConfirmed(true);
      alert(data?.message || "AGENT START BLOCKED — WALLET BASELINE COULD NOT BE CAPTURED");
      return false;
    }
...
async function resetPnlBaseline() {
